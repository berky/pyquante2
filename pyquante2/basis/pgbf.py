...
class pgbf(object):
    ...
    """
    Construct a primitive Gaussian basis function.
    >>> s = pgbf(1.0)
    ...
    def __repr__(self):
        return 'pgbf({}, {}, {})'.format(self.exponent,
                                         tuple(self.origin),
                                         self.powers)

    def __call__(self, x, y, z):
        # What does __call__ do?
        '''Compute the amplitude of this PGBF at point (x, y, z).'''
        I, J, K = self.powers
    ...
    def mesh(self, xyzs):
        ...
        # See: http://stackoverflow.com/questions/17391052/compute-square-distances-from-numpy
        d2 = np.einsum('ij,ij -> i', d, d)
        return self.norm * (d[:, 0]**I) * (d[:, 1]**J) * (d[:, 2]**K) * np.exp(-self.exponent * d2)

    def _normalize(self):
        '''Normalize this basis function. From THO eq. (2.2).'''
        l, m, n = self.powers
